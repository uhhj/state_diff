#!/usr/bin/env python3
"""Temporal frozen-test gate for Phase3.14b-r2.5.8 Stage F."""
...
STAGEF_TEST = "tests/test_phase3_14b_r258_stagef_constraint_aware_surrogate.py"
OUTPUT = "reports/phase3_14b_r258_stagef_test_gate_summary.json"
EXPECTED_STAGEE_PASSED = 81
EXPECTED_STAGEF_PASSED = 84
EXPECTED_TOTAL_FILES = 56
EXPECTED_TOTAL_PASSED = 1408
...
def main() -> None:
    ...
    if output.exists():
        raise FileExistsError("refusing to overwrite Stage-F test gate: {}".format(output))
    ...
    compile_targets = [
        ...
        root / "scripts/phase3_14b_r258_stagef_worker.py",
        root / "scripts/phase3_14b_r258_stagef_run_calibration.py",
        root / "scripts/phase3_14b_r258_stagef_test_gate.py",
        root / "scripts/phase3_14b_r258_stagef_blocked.py",
    ]
    subprocess.run(
    ...
    subprocess.run(
        ["bash", "-n", str(root / "scripts/phase3_14b_r258_stagef_run.sh")],
        cwd=str(root),
        check=True,
    )

    temporary_root = Path(
        tempfile.mkdtemp(prefix="phase314b_r258_stagef_gate_", dir="/tmp")
    )
    ...
    if total_passed != EXPECTED_TOTAL_PASSED:
        raise RuntimeError("Stage-F total pass count changed: {}".format(total_passed))
    combined_manifest = dict(base55_manifest)
    combined_manifest[STAGEF_TEST] = sha256_file(root / STAGEF_TEST)
    if len(combined_manifest) != EXPECTED_TOTAL_FILES:
        raise RuntimeError("Stage-F combined test manifest changed")

    report = {
        "phase": "Phase3.14b-r2.5.8 Stage F",
        "schema": "phase314b_r258_stagef_test_gate_v1",
        "verdict": "PASS",
        "base_evidence_commit": BASE_EVIDENCE_COMMIT,
        "implementation": implementation,
        "base_evidence": {
            "worker_sha256": base_evidence["worker"]["comparison"]["left_sha256"],
            "contract_file_sha256": sha256_file(root / "reports/phase3_14b_r258_stagee_contract.json"),
        ...
        "temporal_test_views": {
            ...
            "test_sources_modified_or_renamed": False,
        },
        ...
        "stagef_population": stagef_result,

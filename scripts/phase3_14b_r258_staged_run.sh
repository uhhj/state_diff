...
on_error() {
  ...
  if [[ "${PUSH_COMPLETED}" != "true" ]]; then
    "${PYTHON_BIN}" \
      "${ROOT}/scripts/phase3_14b_r258_staged_blocked.py" \
      --root "${ROOT}" \
      --exit-code "${code}" \
...
commit_with_outcome() {
  ...
  log_path="$(
    mktemp \
      /tmp/phase314b_r258_staged_commit.XXXXXX.log
  )"
...
push_with_outcome() {
  ...
  log_path="$(
    mktemp \
      /tmp/phase314b_r258_staged_push.XXXXXX.log
  )"
...
result = validate_initial_worktree(root)
print(
    json.dumps(
        result,
        sort_keys=True,
...
PREFLIGHT_DIR="$(
  mktemp -d \
    /tmp/phase314b_r258_staged_environment.XXXXXX
)"
PREFLIGHT="${PREFLIGHT_DIR}/environment.json"
if [[ -e "${PREFLIGHT}" ]]; then
  printf 'Stage-D preflight output unexpectedly exists\n' >&2
  exit 1
fi
...
SUCCESS_PATHS=(
  reports/phase3_14b_r258_staged_test_gate_summary.json
  reports/phase3_14b_r258_staged_contract.json
  reports/phase3_14b_r258_staged_worker_evidence.json
  reports/phase3_14b_r258_staged_summary.json
  reports/phase3_14b_r258_staged_report.md
)
git add "${SUCCESS_PATHS[@]}"
commit_with_outcome \
  "Record Phase3.14b-r2.5.8 direction surrogate evidence" \
  "${SUCCESS_PATHS[@]}"
...
if [[ -n "$(
  ...
  git status --short >&2
  printf 'Main worktree is not clean after Stage-D push\n' >&2
  exit 1
fi
if [[ "$(
  ...
  printf 'Submodule commit changed after Stage D\n' >&2
  exit 1
fi
if [[ -n "$(
  git -C external/deformable-ravens \
  ...
  printf 'Submodule is dirty after Stage D\n' >&2
  exit 1
fi

trap - ERR
printf 'Stage D complete and pushed: %s\n' \
  "${LOCAL_HEAD}"

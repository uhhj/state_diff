#!/usr/bin/env python3
"""Run two isolated Stage-F Resume1 workers and seal evidence."""
...
from ccda_phase3.phase314b_r258_stagef_resume1_translation_fix import (
    ...
    stable_json_bytes,
    status_paths,
    validate_original_blocked_reports,
...
def assert_repository(root: Path, test_gate: Path) -> Dict[str, Any]:
    if git_output(root, "branch", "--show-current") != "Experiment1":
        raise RuntimeError("Stage-F Resume1 requires Experiment1")
    commits = validate_resume1_implementation_commit(root)
    remote = git_output(root, "rev-parse", "origin/Experiment1")
    if remote != BASE_EVIDENCE_COMMIT:
        raise RuntimeError("origin/Experiment1 changed before Resume1 push")
    ...
    if status_paths(root) != expected:
        raise RuntimeError(
            "unexpected worktree paths before Resume1 scientific run: {}".format(
                status_paths(root)
            )
        )
    failure = validate_original_blocked_reports(root)
    return {
        "branch": "Experiment1",
...
def render_resume1_report(summary: Dict[str, Any], original_renderer: Any) -> str:
    ...
    prefix = [
        "# Phase3.14b-r2.5.8 Stage F Resume1",
        "",
        ...
        "- Blocked provenance commit: `{}`".format(
            commits["blocked_provenance_commit"]
        ),
        "- Resume1 implementation commit: `{}`".format(
            commits["resume1_implementation_commit"]
        ),
        "- Original calibration completed before Resume1: `false`",
...
def main() -> None:
    ...
    parser.add_argument("--contract", default=RESUME1_CONTRACT)
    parser.add_argument("--worker-evidence", default=RESUME1_WORKER)
    parser.add_argument("--summary", default=RESUME1_SUMMARY)
    parser.add_argument("--report", default=RESUME1_REPORT)
    ...
    test_gate_path = root / RESUME1_TEST_GATE
    for path in (contract_path, worker_path, summary_path, report_path):
        if path.exists():
            raise FileExistsError("Resume1 write-once output exists: {}".format(path))

    repository = assert_repository(root, test_gate_path)
    test_gate = load_json(test_gate_path)
    if test_gate.get("verdict") != "PASS":
        raise RuntimeError("Stage-F Resume1 test gate is not PASS")
    if int(test_gate.get("test_file_count", -1)) != 57:
        raise RuntimeError("Stage-F Resume1 test-file count changed")
    if int(test_gate.get("passed_test_count", -1)) != 1455:
        raise RuntimeError("Stage-F Resume1 pass count changed")

    original_renderer = load_original_renderer(root)
    temporary_root = Path(
        tempfile.mkdtemp(prefix="phase314b_r258_stagef_resume1_workers_", dir="/tmp")
    )
    worker_files = [temporary_root / "worker_1.json", temporary_root / "worker_2.json"]
    command = [
        str(args.python_bin),
        str(root / "scripts/phase3_14b_r258_stagef_resume1_worker.py"),
        "--root",
        str(root),
        "--mode",
        "run",
    ...
    try:
        ...
        if not comparison["exact"]:
            raise RuntimeError("isolated Stage-F Resume1 workers differ")
        result = worker_results[0]

        correction = {
            "schema": "phase314b_r258_stagef_resume1_translation_fix_v1",
            "original_failure": (
            ...
            "test_gate_path": RESUME1_TEST_GATE,
        ...
        contract_record = {
            **result["constraint_aware_contract"],
            "phase": PHASE,
            "schema": "phase314b_r258_stagef_resume1_contract_record_v1",
            "resume1_translation_fix": correction,
        ...
        worker_evidence = {
            "phase": PHASE,
            "schema": "phase314b_r258_stagef_resume1_worker_evidence_v1",
            "worker_count": 2,
            "workers_exact": True,
        ...
        summary = {
            "phase": PHASE,
            "schema": "phase314b_r258_stagef_resume1_summary_v1",
            "verdict": "PASS",
            "scientific_status": result["scientific_status"],
            "root_cause": result["root_cause"],
            "required_next_path": result["required_next_path"],

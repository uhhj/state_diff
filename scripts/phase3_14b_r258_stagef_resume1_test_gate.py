#!/usr/bin/env python3
"""Temporal frozen-test gate for Stage-F Resume1."""
...
EXPECTED_TOTAL_FILES = 57
EXPECTED_TOTAL_PASSED = 1455
...
def main() -> None:
    ...
    parser.add_argument("--output", default=RESUME1_TEST_GATE)
    ...
    if output.exists():
        raise FileExistsError(
            "refusing to overwrite Stage-F Resume1 test gate: {}".format(output)
        )

    implementation = validate_resume1_implementation_commit(root)
    failure_evidence = validate_original_blocked_reports(root)
    ...
    subprocess.run(
        ["bash", "-n", str(root / "scripts/phase3_14b_r258_stagef_resume1_run.sh")],
        cwd=str(root),
        check=True,
    )

    temporary_root = Path(
        tempfile.mkdtemp(prefix="phase314b_r258_stagef_resume1_gate_", dir="/tmp")
    )
    ...
    if total_passed != EXPECTED_TOTAL_PASSED:
        raise RuntimeError("Stage-F Resume1 total pass count changed: {}".format(total_passed))
    ...
    if len(combined_manifest) != EXPECTED_TOTAL_FILES:
        raise RuntimeError("Stage-F Resume1 combined test manifest changed")

    report = {
        "phase": "Phase3.14b-r2.5.8 Stage F Resume1",
        "schema": "phase314b_r258_stagef_resume1_test_gate_v1",
        "verdict": "PASS",
        "base_evidence_commit": BASE_EVIDENCE_COMMIT,
        "implementation": implementation,
        "blocked_provenance": {
            "test_gate_sha256": failure_evidence["test_gate_sha256"],
            "blocked_sha256": failure_evidence["blocked_sha256"],

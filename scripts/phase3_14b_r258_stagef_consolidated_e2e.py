...
def validate_repository(repo: Path) -> Mapping[str, Any]:
    ...
    stagee_hashes = {
        ...
        for relpath in STAGEE_IMMUTABLE_PATHS
    }
    stagee_hashes[STAGEE_SCIENTIFIC_PATH] = sha256_path(
        repo / STAGEE_SCIENTIFIC_PATH
    )
    stagee_hashes[STAGEE_SCIENTIFIC_PATH + ":parent_blob"] = sha256_bytes(
        git_bytes(
            repo,
            "show",
            f"{EXPECTED_PARENT}:{STAGEE_SCIENTIFIC_PATH}",
        )
    )

...
BLOCKED_JSON="reports/phase3_14b_r254_resume3_blocked_summary.json"
BLOCKED_MD="reports/phase3_14b_r254_resume3_blocked_report.md"
...
write_blocked() {
  ...
  if [[ -e "${BLOCKED_JSON}" || -e "${BLOCKED_MD}" ]]; then
    printf 'Resume3 blocked evidence already exists; refusing overwrite.\n' >&2
    return 0
  fi
  if [[ -x "${PYTHON_BIN}" ]]; then
    "${PYTHON_BIN}" scripts/phase3_14b_r254_resume3_blocked.py \
      --root "${ROOT}" \
      --exit-code "${exit_code}" \
...
print("Resume3 interpreter, CUDA, repository import, and prediction adapter: PASS")
print(CURRENT_DEVICE_PRIOR_STATE_SHA256)
print(CURRENT_DEVICE_PRIOR_PREDICTION_SHA256)
PY

"${PYTHON_BIN}" scripts/phase3_14b_r254_resume3_preflight.py \
  --root "${ROOT}" \
  --output reports/phase3_14b_r254_resume3_preflight_summary.json

"${PYTHON_BIN}" scripts/phase3_14b_r254_resume3_run_pilot.py \
  --root "${ROOT}" \
  --preflight-report reports/phase3_14b_r254_resume3_preflight_summary.json \
  --output reports/phase3_14b_r254_resume3_pilot_summary.json

"${PYTHON_BIN}" scripts/phase3_14b_r254_resume3_finalize.py \
  --root "${ROOT}" \
  --preflight-report reports/phase3_14b_r254_resume3_preflight_summary.json \
  --pilot-report reports/phase3_14b_r254_resume3_pilot_summary.json \
  --summary-output reports/phase3_14b_r254_summary.json \
  --markdown-output reports/phase3_14b_r254_report.md

trap - ERR
printf 'Phase3.14b-r2.5.4 Resume3 completed.\n'

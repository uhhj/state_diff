#!/usr/bin/env python3
"""Preflight for r2.5.3 Resume3 finalizer-only recovery."""
...
def main() -> None:
    ...
    parser.add_argument("--output", default=RESUME3_PREFLIGHT_RELATIVE)
    ...
    if output_relative != RESUME3_PREFLIGHT_RELATIVE:
        raise RuntimeError("Resume3 preflight output path changed")

    assert_commit_is_ancestor(root, BASE_RESUME2_BLOCKED_COMMIT)
    assert_only_allowed_paths(root, ())
    validate_resume3_outputs_absent(root)
    ...
    payload = {
        "phase": PHASE,
        "verdict": "PASS",
        "meaning": "Resume3 finalizer-only interpreter preflight completed",
        "resume": {
            "enabled": True,
            "generation": 3,
            "schema": RESUME3_PROVENANCE_SCHEMA,
            ...
            "correction": "default_python_not_bound_to_coord_bimanual",
        },
        ...
        "resume2_evidence_sha256": resume2_hashes,

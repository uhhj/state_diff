...
PREFLIGHT="reports/phase3_14b_r252_preflight_summary.json"
PILOT="reports/phase3_14b_r252_pilot_summary.json"
SUMMARY="reports/phase3_14b_r252_summary.json"
REPORT="reports/phase3_14b_r252_report.md"
BLOCKED_JSON="reports/phase3_14b_r252_blocked_summary.json"
BLOCKED_MD="reports/phase3_14b_r252_blocked_report.md"
...
for path in (json_path, md_path):
    ...
    if path.exists():
        raise SystemExit(f"refusing to overwrite blocked report: {path}")
traceback = traceback_path.read_text(encoding="utf-8", errors="replace")[-16000:]
pilot_path = root / "reports/phase3_14b_r252_pilot_summary.json"
...
report = {
    ...
    "verdict": "BLOCKED",
    "root_cause": "phase314b_r252_execution_failed",
    "exit_code": exit_code,
    "stage": stage,
...
md_path.write_text(
    "# Phase3.14b-r2.5.2 Blocked Report\n\n"
    "- Verdict: `BLOCKED`\n"
    "- Root cause: `phase314b_r252_execution_failed`\n"
...
for path in "${PREFLIGHT}" "${PILOT}" "${SUMMARY}" "${REPORT}" "${BLOCKED_JSON}" "${BLOCKED_MD}"; do
  if [[ -e "${path}" ]]; then
    echo "Refusing to overwrite existing r2.5.2 artifact: ${path}" >&2
    exit 1
  fi
done

run_stage preflight \
  python scripts/phase3_14b_r252_preflight.py \
    --root "${ROOT}" \
    --output "${PREFLIGHT}"

run_stage pilot \
  python scripts/phase3_14b_r252_run_pilot.py \
    --root "${ROOT}" \
...
run_stage finalize \
  python scripts/phase3_14b_r252_finalize.py \
    --root "${ROOT}" \
    --preflight-report "${PREFLIGHT}" \
    --pilot-report "${PILOT}"

...
def scan_scripts(root: Path) -> List[Dict[str, Any]]:
    ...
    targets = [
        ...
        root / "scripts/phase3_12b_proxy_score_rollout.py",
        root / "scripts/phase3_12b_analyze_proxy_score.py",
    ]
    ...
    for path in targets:
        ...
        for term in forbidden_input_terms:
            for match in re.finditer(term, txt):
                ...
                if any(k in ctx for k in ["model_x", "idm_x", "policy_input", "torch.cat", "input_vec"]):
                    findings.append({
                        "level": "FAIL",
...
def main() -> None:
    ...
    parser.add_argument("--phase312_summary", default="reports/phase3_12_compat_selection_summary.json")
    parser.add_argument("--best_ablation", default="xy_only_high_weight")
    parser.add_argument("--out_json", default="reports/phase3_12b_proxy_score_preflight_summary.json")
    parser.add_argument("--out_md", default="reports/phase3_12b_proxy_score_preflight_report.md")
    ...
    # Reuse the TensorFlow-free minimal runtime used by the established
    # rollout diagnostics before probing ravens.tasks/environment.
    try:
    ...
    p312 = load_json(root / args.phase312_summary)
    flags = p312.get("diagnostic_flags", {})
    checks["phase312_summary_exists"] = bool(p312)
    checks["phase312_condition_upper_bound_supported"] = bool(flags.get("simple_condition_supported", False))
    checks["phase312_input_nearest_not_supported"] = not bool(flags.get("simple_input_supported", False))
    checks["phase312_compat_global_not_supported"] = not bool(flags.get("compat_global_supported", False))
    checks["phase312_no_phase4_scope"] = (root / "reports/phase3_12_no_phase4_confirmation.md").exists()

    for key in [
        "phase312_summary_exists",
        "phase312_condition_upper_bound_supported",
        "phase312_input_nearest_not_supported",
        "phase312_compat_global_not_supported",
        "phase312_no_phase4_scope",
    ]:
        if not checks[key]:
            issue("FAIL", key, key)

    root_cause = p312.get("root_cause")
    if root_cause == "phase312_simple_locality_selection_supported":
        issue("WARN", "phase312_root_cause_label_misleading", "Phase3.12 table supports condition_nearest upper bound, not input-only simple locality.")
    ...
    if verdict == "FAIL":
        raise SystemExit("[Phase3.12b][FAIL] preflight failed")

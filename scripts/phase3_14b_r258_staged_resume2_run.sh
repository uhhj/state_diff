...
on_error() {
  ...
  if [[ "${PUSH_COMPLETED}" != "true" ]]; then
    "${PYTHON_BIN}" \
      "${ROOT}/scripts/phase3_14b_r258_staged_resume2_blocked.py" \
      --root "${ROOT}" \
      --exit-code "${code}" \
...
commit_with_outcome() {
  ...
  log_path="$(
    mktemp \
      /tmp/phase314b_r258_staged_resume2_commit.XXXXXX.log
  )"
...
push_with_outcome() {
  ...
  log_path="$(
    mktemp \
      /tmp/phase314b_r258_staged_resume2_push.XXXXXX.log
  )"
...
from ccda_phase3.phase314b_r258_staged_resume2_temporal_views import (
    validate_resume1_blocked_provenance_commit,
)

print(
    json.dumps(
        validate_resume1_blocked_provenance_commit(root),
        sort_keys=True,
    )
)
PY
...
PREFLIGHT_DIR="$(
  mktemp -d \
    /tmp/phase314b_r258_staged_resume2_environment.XXXXXX
)"
PREFLIGHT="${PREFLIGHT_DIR}/environment.json"
if [[ -e "${PREFLIGHT}" ]]; then
  printf 'Resume2 preflight output unexpectedly exists\n' >&2
  exit 1
fi
...
trap - ERR
printf 'Stage-D Resume2 complete and pushed: %s\n' \
  "${LOCAL_HEAD}"

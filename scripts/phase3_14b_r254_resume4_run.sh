...
on_error() {
  local code="$?"
  "${PYTHON_BIN}" "${ROOT}/scripts/phase3_14b_r254_resume4_blocked.py" \
    --root "${ROOT}" \
    --exit-code "${code}" \
...
print({"python": sys.version.split()[0], "numpy": np.__version__, "torch": torch.__version__, "cuda": torch.version.cuda, "gpu": torch.cuda.get_device_name(0)})
PY

"${PYTHON_BIN}" scripts/phase3_14b_r254_resume4_test_gate.py --root "${ROOT}"
"${PYTHON_BIN}" scripts/phase3_14b_r254_resume4_preflight.py --root "${ROOT}"
"${PYTHON_BIN}" scripts/phase3_14b_r254_resume4_audit.py --root "${ROOT}"
"${PYTHON_BIN}" scripts/phase3_14b_r254_resume4_finalize.py --root "${ROOT}"

#!/usr/bin/env python3
"""Controller for Stage-B Resume1 post-commit evidence-audit recovery."""
...
from ccda_phase3.phase314b_r259_stageb_resume1_postcommit_evidence_audit_recovery import (
    BLOCKED_REPORT,
    SUCCESS_REPORT,
    blocked_report,
    run_recovery,
...
def main() -> int:
    ...
    if success.exists() or blocked.exists():
        print("BLOCKED: Stage-B Resume1 terminal output already exists", file=sys.stderr)
        return 2

    repository = None
    try:
        # run_recovery performs the single repository validation.  Do not call
        # validate_repository separately here; that duplication caused avoidable
        # contract complexity in the original controller.
        result = run_recovery(root, args.implementation_commit)

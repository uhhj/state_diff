...
def main() -> int:
    ...
    parser.add_argument("--finalize-existing-evidence", action="store_true")
    ...
    write_ahead = (
        ...
        else root.parent / (root.name + ".phase314b_r259_stagea_write_ahead")
    )
    try:
    ...
    if success.exists() or blocked.exists():
        print("BLOCKED: Stage-A terminal report already exists", file=sys.stderr)
        return 2
    repository = None
    try:
        repository = validate_repository(
            root, allow_existing_write_ahead=args.finalize_existing_evidence
        )
        if args.finalize_existing_evidence:
            if not ahead_probe.is_file() or not ahead_worker.is_file():
                raise RuntimeError("external durable probe/worker evidence is incomplete")
        else:
            if write_ahead.exists():
                raise RuntimeError(
                    "write-ahead directory already exists; use --finalize-existing-evidence "
                    "without launching new science"
                )
            write_ahead.mkdir(parents=True, exist_ok=False)
            env = child_environment(root)
            script = root / "scripts/phase3_14b_r259_stagea_worker.py"
            run_child(
                [
                    args.python_bin,
                    str(script),
                ...
                root=root,
                env=env,
                label="external durable environment probe",
            )
            run_child(
                [
                    args.python_bin,
                ...
                root=root,
                env=env,
                label="external durable cold science worker",
            )
        ...
        # Preserve valid external results by promoting them before writing the blocked report.
        try:
            if ahead_probe.is_file():
                promote_write_ahead(ahead_probe, repo_probe, validate_probe_evidence)
            if ahead_worker.is_file():
                promote_write_ahead(ahead_worker, repo_worker, validate_worker_evidence)

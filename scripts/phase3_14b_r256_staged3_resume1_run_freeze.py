#!/usr/bin/env python3
"""Run two isolated Stage-D.3 Resume1 workers and seal evidence."""
...
def assert_repository(root: Path, test_gate: Path) -> Dict[str, Any]:
    if git_output(root, "branch", "--show-current") != "Experiment1":
        raise RuntimeError("Stage D.3 Resume1 requires Experiment1")
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            "blocked Stage-D.3 implementation is not an ancestor"
        )
...
def render_report(summary: Dict[str, Any]) -> str:
    ...
    blocked = result["immutable_inputs"]["blocked_attempt"]
    lines = [
        ...
        (
            ...
            + blocked["implementation_commit"]
            + "`"
        ),
        (
            "- Blocked evidence commit: `"
            + blocked["blocked_evidence_commit"]
            + "`"
        ),
        (
            "- Original test-gate SHA256: `"
            + blocked["test_gate_sha256"]
            + "`"
        ),
        (
            "- Original blocked-summary SHA256: `"
            + blocked["blocked_summary_sha256"]
            + "`"
        ),
...
def main() -> None:
    ...
    for path in (
        ...
        if path.exists():
            raise FileExistsError(
                f"Resume1 write-once output exists: {path}"
            )

    repository = assert_repository(root, test_gate_path)
    test_gate = load_json(test_gate_path)
    if test_gate.get("verdict") != "PASS":
        raise RuntimeError("Resume1 test gate is not PASS")

    temporary_root = Path(
        tempfile.mkdtemp(
            prefix="phase314b_r256_staged3_resume1_",
            dir="/tmp",
        )
    )
    worker_files = [
    ...
    if not comparison["exact"]:
        raise RuntimeError(
            "isolated Stage-D.3 Resume1 workers differ"
        )
    ...
    worker_evidence = {
        "phase": PHASE,
        "schema":
            "phase314b_r256_staged3_resume1_worker_evidence_v1",
        "worker_count": 2,
        "workers_exact": True,
    ...
    summary = {
        "phase": PHASE,
        "schema": "phase314b_r256_staged3_resume1_summary_v1",
        "verdict": "PASS",
        ...
        "worker_result": result,
        "blocked_attempt_preserved": True,
        "source_predicate_corrected": True,

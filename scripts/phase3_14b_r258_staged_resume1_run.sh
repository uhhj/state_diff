...
on_error() {
  ...
  if [[ "${PUSH_COMPLETED}" != "true" ]]; then
    "${PYTHON_BIN}" \
      "${ROOT}/scripts/phase3_14b_r258_staged_resume1_blocked.py" \
      --root "${ROOT}" \
      --exit-code "${code}" \
...
commit_with_outcome() {
  ...
  log_path="$(
    mktemp \
      /tmp/phase314b_r258_staged_resume1_commit.XXXXXX.log
  )"
...
push_with_outcome() {
  ...
  log_path="$(
    mktemp \
      /tmp/phase314b_r258_staged_resume1_push.XXXXXX.log
  )"
...
print(
    json.dumps(
        validate_initial_worktree(root),
        sort_keys=True,
    )
)
PY
...
git add "${ORIGINAL_IMPLEMENTATION_PATHS[@]}"
commit_with_outcome \
...
from ccda_phase3.phase314b_r258_staged_resume1_test_view import (
    validate_original_implementation_commit,
)

print(
    json.dumps(
        validate_original_implementation_commit(root),
        sort_keys=True,
    )
)
PY
...
print(
    json.dumps(
        validate_blocked_provenance_commit(root),
        sort_keys=True,
    )
)
PY
...
print(
    json.dumps(
        validate_resume1_implementation_commit(root),
        sort_keys=True,
    )
)
PY
...
PREFLIGHT_DIR="$(
  mktemp -d \
    /tmp/phase314b_r258_staged_resume1_environment.XXXXXX
)"
...
SUCCESS_PATHS=(
  reports/phase3_14b_r258_staged_resume1_test_gate_summary.json
  reports/phase3_14b_r258_staged_resume1_contract.json
  reports/phase3_14b_r258_staged_resume1_worker_evidence.json
  reports/phase3_14b_r258_staged_resume1_summary.json
  reports/phase3_14b_r258_staged_resume1_report.md
)
...
trap - ERR
printf 'Stage-D Resume1 complete and pushed: %s\n' \
  "${LOCAL_HEAD}"

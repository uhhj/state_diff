#!/usr/bin/env python3
"""Controller and child entry point for Stage-D Resume2."""
...
def main() -> int:
    ...
    from ccda_phase3.phase314b_r259_staged_resume2_cable_state_schema_recovery import (
        BLOCKED_REPORT,
        PROBE_EVIDENCE,
        SUCCESS_REPORT,
        WORKER_EVIDENCE,
    ...
    write_ahead = (
        ...
        else root.parent / (root.name + ".phase314b_r259_staged_resume2_write_ahead")
    )
    try:
    ...
    if success.exists() or blocked.exists():
        print("BLOCKED: Resume2 terminal report already exists", file=sys.stderr)
        return 2
    ...
    try:
        ...
        if write_ahead.exists():
            raise RuntimeError("Resume2 write-ahead already exists")
        ...
        run_child(
            ...
            root=root,
            env=env,
            label="Stage-D Resume2 portable probe",
        )
        run_child(
            [
                args.python_bin,
            ...
            root=root,
            env=env,
            label="Stage-D Resume2 cold science worker",
        )

#!/usr/bin/env python3
"""Finalize r2.5.4 Resume3 robot-proxy attribution."""
...
def load_original_finalizer(root: Path) -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location("phase314b_r254_original_finalizer_r3", path)
...
def render_report(original_module: ModuleType, summary: Mapping[str, Any]) -> str:
    ...
    appendix = [
        "## Resume3 functional-prior prediction reconstruction",
        "",
        ...
        f"- Fresh prior z-MSE pass: `{str(fresh['prior_z_mse_pass']).lower()}`",
        f"- Prediction reconstructed from retained state: `{str(fresh['prediction_reconstructed_from_prior_state']).lower()}`",
        f"- Stochastic state restored: `{str(fresh['rng_state_restored']).lower()}`",
        ...
        "- Same-device exact state/prediction SHA required: `True`",
        "",
    ]
    return base.rstrip() + "\n\n" + "\n".join(appendix)
...
def main() -> None:
    ...
    parser.add_argument("--pilot-report", default=RESUME3_PILOT_PATH)
    ...
    summary: Dict[str, Any] = {
        ...
        "resume_generation": 3,
        "resume3_schema": RESUME3_SCHEMA,
        "verdict": "PASS",
        "meaning": (
            "train-only robot-proxy attribution completed under the audited "
            "functional-prior and pure-memory prediction contracts"
        ),
        ...
        "prior_prediction_reconstructed_in_memory": True,
        "prior_prediction_tensor_persisted": False,

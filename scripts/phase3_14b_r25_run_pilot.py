#!/usr/bin/env python3
"""Run Phase3.14b-r2.5 train-only frozen-prior geometry pilot."""
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("r2.5 pilot requires CUDA")
    device = torch.device("cuda")

    preflight = load_json(preflight_path)
    if preflight.get("verdict") != "PASS":
        raise RuntimeError("r2.5 preflight did not pass")
    if preflight.get("r25_source_sha256") != source_sha256(root):
        raise RuntimeError("r2.5 source changed after preflight")
    ...
    common_unique_seed = COMMON_UNIQUE_TRAINING_SEED
    for objective in GEOMETRY_OBJECTIVES:
        unique_runtime[objective.name] = train_geometry_variant(
            scheduler=scheduler,
            **unique_data,
            ...
            seed=common_unique_seed,
        )

    advancing_names = [
        objective.name
        for objective in GEOMETRY_OBJECTIVES
        if bool(unique_runtime[objective.name]["pass"])
    ]
    ...
    objective_by_name = {objective.name: objective for objective in GEOMETRY_OBJECTIVES}
    common_paired_seed = COMMON_PAIRED_TRAINING_SEED
    common_reverse_seed = COMMON_REVERSE_SEED
    for name in advancing_names:
        objective = objective_by_name[name]
        result = train_geometry_variant(
            scheduler=scheduler,
            **paired_data,
        ...
        reverse_metrics_raw = paired_reverse_pool_metrics(
            pool_z=pool_z,
        ...
        result["reverse_metrics"] = reverse_metrics
    ...
    if control_name in paired_runtime:
        ...
        control_compact = {
            "reverse_metrics": control["reverse_metrics"],
        }
        for name, result in paired_runtime.items():
            if name == control_name:
                ...
                result["comparison_to_control"] = compare_geometry_to_control(
    ...
    report: Dict[str, Any] = {
        "phase": PHASE,
        "verdict": "PASS",
        "meaning": "train-only frozen-p512/r512 ordered-geometry pilot completed; no formal selection",
        ...
        "evaluation": {
            ...
            "common_unique_training_seed": common_unique_seed,
            "common_paired_training_seed": common_paired_seed,
            "common_reverse_seed": common_reverse_seed,
            "unique_one_step_timesteps": list(UNIQUE_EVAL_TIMESTEPS),
            "paired_one_step_timesteps": [10, 25, 50],
        ...
        "fixed_model_contract": {
            ...
            "prior_policy": "strict_frozen_after_warmup",
            "batching": "balanced",
    ...
    write_json_once(root / "reports/phase3_14b_r25_pilot_summary.json", report)
    print(
        json.dumps(
            {
                ...
                "paired_one_step_pass": {
                    key: bool(value["one_step_pass"])
                    for key, value in paired_results.items()
                },
                "paired_full_reverse_branch_support": {
                    key: float(value["full_reverse_branch_support"]["both_branch_support_rate"])
                    for key, value in paired_results.items()
                },
            },
            indent=2,

#!/usr/bin/env python3
"""Execute Stage-Q tolerance-aligned upper-gate shadow audit exactly once."""
...
from ccda_phase3.phase314b_r258_stageq_tolerance_aligned_upper_gate_shadow import (
    BLOCKED_REPORT,
    SUCCESS_REPORT,
    StageQError,
    blocked_report,
    run_shadow_alignment,
    stable_json_bytes,
...
def load_json(path: Path) -> Mapping[str, Any]:
    ...
    if not isinstance(value, dict):
        raise StageQError("environment payload is not a mapping")
    return value
...
def write_once(path: Path, payload: bytes) -> None:
    target = Path(path)
    if target.exists():
        raise StageQError(f"write-once output already exists: {target}")
...
def main() -> int:
    ...
    if success.exists() or blocked.exists():
        print("BLOCKED: Stage-Q output already exists", file=sys.stderr)
        return 2

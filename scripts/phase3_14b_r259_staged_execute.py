#!/usr/bin/env python3
"""Controller for durable Stage-D t=10 risk repair evidence."""
...
def main() -> int:
    ...
    write_ahead = (
        ...
        else root.parent / (root.name + ".phase314b_r259_staged_write_ahead")
    )
    try:
    ...
    if success.exists() or blocked.exists():
        print("BLOCKED: Stage-D terminal report already exists", file=sys.stderr)
        return 2
    ...
    try:
        ...
        if write_ahead.exists():
            raise RuntimeError("Stage-D write-ahead directory already exists; do not rerun science")
        write_ahead.mkdir(parents=True, exist_ok=False)
        env = child_environment(root)
        script = root / "scripts/phase3_14b_r259_staged_worker.py"
        run_child(
            [
                args.python_bin,
                str(script),
            ...
            root=root,
            env=env,
            label="external durable Stage-D portable environment probe",
        )
        for slot, output in (("A", ahead_worker_a), ("B", ahead_worker_b)):
            run_child(
                ...
                root=root,
                env=env,
                label="external durable Stage-D cold science worker {}".format(slot),
            )

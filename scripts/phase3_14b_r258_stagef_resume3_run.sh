...
on_error() {
  ...
  if [[ "${PUSH_COMPLETED}" != "true" ]]; then
    "${PYTHON_BIN}" \
      "${ROOT}/scripts/phase3_14b_r258_stagef_resume3_blocked.py" \
      --root "${ROOT}" \
      --exit-code "${code}" \
...
from ccda_phase3.phase314b_r258_stagef_resume2_porcelain_recovery import validate_initial_worktree
...
[[ ! -e "${PREFLIGHT}" ]]
"${PYTHON_BIN}" scripts/phase3_14b_r258_stagef_resume3_worker.py \
...
"${PYTHON_BIN}" scripts/phase3_14b_r258_stagef_resume3_test_gate.py \
  --root "${ROOT}" --python-bin "${PYTHON_BIN}"
"${PYTHON_BIN}" scripts/phase3_14b_r258_stagef_resume3_run_calibration.py \
  --root "${ROOT}" --python-bin "${PYTHON_BIN}"

SUCCESS_PATHS=(
  reports/phase3_14b_r258_stagef_resume3_test_gate_summary.json
  reports/phase3_14b_r258_stagef_resume3_contract.json
  reports/phase3_14b_r258_stagef_resume3_worker_evidence.json
  reports/phase3_14b_r258_stagef_resume3_summary.json
  reports/phase3_14b_r258_stagef_resume3_report.md
)

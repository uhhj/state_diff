...
commit_with_outcome() {
  ...
  log_path="$(mktemp /tmp/phase314b_r258_stagef_resume1_commit.XXXXXX.log)"
  ...
  actual_paths="$(
    git diff-tree --no-commit-id --name-only -r "${post_head}" | LC_ALL=C sort
  )"
...
push_with_outcome() {
  ...
  log_path="$(mktemp /tmp/phase314b_r258_stagef_resume1_push.XXXXXX.log)"
  ...
  for attempt in 1 2 3; do
    set +e
    remote_head="$(
      git ls-remote origin refs/heads/Experiment1 2>/dev/null | awk '{print $1}'
    )"
    set -e
...
sys.path.insert(0, str(root))
from ccda_phase3.phase314b_r258_stagef_resume1_translation_fix import (
    validate_blocked_provenance_commit,
)
print(json.dumps(validate_blocked_provenance_commit(root), sort_keys=True))
PY
...
PREFLIGHT_DIR="$(mktemp -d /tmp/phase314b_r258_stagef_resume1_environment.XXXXXX)"
...
print({
    ...
    "peak_reserved_bytes": value["required_operation_dry_run"][
...
SUCCESS_PATHS=(
  ...
  reports/phase3_14b_r258_stagef_resume1_contract.json
  reports/phase3_14b_r258_stagef_resume1_worker_evidence.json
  reports/phase3_14b_r258_stagef_resume1_summary.json
  reports/phase3_14b_r258_stagef_resume1_report.md
)
...
[[ -z "$(git -C external/deformable-ravens status --porcelain --untracked-files=all)" ]] || {
  printf 'Submodule is dirty\n' >&2
  exit 1
}

trap - ERR
printf 'Stage-F Resume1 complete and pushed: %s\n' "${LOCAL_HEAD}"

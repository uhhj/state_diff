...
def scan_scripts(root: Path) -> List[Dict[str, Any]]:
    ...
    targets = [
        ...
        root / "scripts/phase3_11_future_source_swap_rollout.py",
        root / "scripts/phase3_11b_retrieval_feasibility_probe.py",
        root / "scripts/phase3_11b_analyze_retrieval_feasibility.py",
    ]
        ...
        for term in forbidden_input_terms:
            for match in re.finditer(term, txt):
                ...
                if any(k in ctx for k in [
                    "EPISODE_FIELDS", "STEP_FIELDS", "_FIELDS =", "Do not concatenate",
                    "select_condition_retrieval", "data[\"condition_name\"]",
                    "final_fraction_from_info", "fraction_before", "exec_final_fraction",
                    "mean_delta", "mean_final_after", "worker_stderr_tail",
                    "\"status\", \"started_at\"", "\"success\", \"initial_fraction\"",
                ]):
                    continue
...
def main() -> None:
    ...
    parser.add_argument("--phase311_summary", default="reports/phase3_11_future_source_swap_summary.json")
    parser.add_argument("--phase311_steps", default="reports/phase3_11_future_source_swap_steps.csv")
    parser.add_argument("--best_ablation", default="xy_only_high_weight")
    parser.add_argument("--out_json", default="reports/phase3_11b_retrieval_feasibility_preflight_summary.json")
    parser.add_argument("--out_md", default="reports/phase3_11b_retrieval_feasibility_preflight_report.md")
    ...
    # Reuse the existing TensorFlow-free DeformableRavens runtime loader.
    # Direct ravens.tasks import can execute top-level modules that depend on
    # TensorFlow, which Phase3.11b intentionally does not install.
    try:
    ...
    for name in [
        ...
        "phase3_11_future_source_swap_rollout",
        "phase3_8_idm_geometry_repair_probe",
    ]:
    ...
    p311 = load_json(root / args.phase311_summary)
    checks["phase311_summary_exists"] = bool(p311)
    checks["phase311_inconclusive"] = p311.get("root_cause") == "phase311_future_source_swap_inconclusive_or_not_supported"
    checks["phase311_steps_exists"] = (root / args.phase311_steps).exists()
    checks["phase311_no_phase4_scope"] = (root / "reports/phase3_11_no_phase4_confirmation.md").exists()

    for key in [
        "phase311_summary_exists",
        "phase311_inconclusive",
        "phase311_steps_exists",
        "phase311_no_phase4_scope",
    ]:
        if not checks[key]:
            issue("FAIL", key, key)
    ...
    payload = {
        ...
        "shapes": shapes,
        "phase311_summary": {
            "verdict": p311.get("verdict"),
            "root_cause": p311.get("root_cause"),
            "diagnostic_flags": p311.get("diagnostic_flags"),
        },
        "checkpoints": {
    ...
    if verdict == "FAIL":
        raise SystemExit("[Phase3.11b][FAIL] preflight failed")

...
def install_ravens_stub(defravens: Path) -> None:
    ...
    Phase3.10 preflight only needs task/environment registration checks.
    """
...
def scan_scripts(root: Path) -> List[Dict[str, Any]]:
    ...
    import_scan_targets = [
        ...
        root / "scripts/phase3_10_controlled_learned_rollout.py",
        root / "scripts/phase3_10_analyze_learned_rollout.py",
    ]
    # Only scripts that construct rollout/model inputs are scanned for forbidden
    # metadata-in-input hazards. The analyzer only reads output CSV/JSON.
    input_scan_targets = [
        root / "scripts/phase3_policy_rollout.py",
        root / "scripts/phase3_10_controlled_learned_rollout.py",
    ]
    ...
    for path in input_scan_targets:
        ...
        for term in forbidden_input_terms:
            for match in re.finditer(term, txt):
                ctx = txt[max(0, match.start() - 220): min(len(txt), match.end() + 220)]
...
def main() -> None:
    ...
    parser.add_argument("--out_json", default="reports/phase3_10_learned_rollout_preflight_summary.json")
    parser.add_argument("--out_md", default="reports/phase3_10_learned_rollout_preflight_report.md")
    ...
    checks["phase39b_no_phase4_scope"] = "No Phase4" in (root / "reports/phase3_9b_no_phase4_confirmation.md").read_text(errors="replace") if (root / "reports/phase3_9b_no_phase4_confirmation.md").exists() else False

    for key in [
        "phase39b_summary_exists",
        "phase39b_default_robust",
        "phase39b_best_matches",
        "phase39b_no_phase4_scope",
    ]:
        if not checks[key]:
            issue("FAIL", key, key)
    ...
    payload = {
        ...
        "phase39b_summary": {
            "verdict": p39b.get("verdict"),
            "root_cause": p39b.get("root_cause"),
            "best_ablation": p39b.get("best_ablation"),
            "default_robust": p39b.get("default_robust"),
        },
    }
    ...
    lines = [
        "# Phase3.10 Controlled Learned Rollout Preflight",
        "",
        "## Verdict",
        "",
        f"- Verdict: `{verdict}`",
    ...
    lines += [
        ...
        "- Phase3.10 runs controlled learned rollout retry only.",
        "- No model training.",
    ...
    if verdict == "FAIL":
        raise SystemExit("[Phase3.10][FAIL] preflight failed")

#!/usr/bin/env python3
"""Execute Stage-Q Resume1 candidate hash-domain recovery exactly once."""
...
from ccda_phase3.phase314b_r258_stageq_resume1_candidate_hash_domain_recovery import (
    BLOCKED_REPORT,
    SUCCESS_REPORT,
    StageQResume1Error,
    blocked_report,
...
def load_json(path: Path) -> Mapping[str, Any]:
    ...
    if not isinstance(value, dict):
        raise StageQResume1Error("environment payload is not a mapping")
    return value
...
def write_once(path: Path, payload: bytes) -> None:
    target = Path(path)
    if target.exists():
        raise StageQResume1Error(f"write-once output already exists: {target}")
...
def main() -> int:
    ...
    if success.exists() or blocked.exists():
        print("BLOCKED: Stage-Q Resume1 output already exists", file=sys.stderr)
        return 2
            ...
            json.dumps(
                {
                    ...
                    "stage_q_result_sha256": result["stage_q_result_sha256"],
                    "stage_q_scientific_result_sha256": result[
                        "stage_q_scientific_result_sha256"
                    ],
                    "output": str(success),
                },
                sort_keys=True,

#!/usr/bin/env python3
"""Controller and child entry point for Stage-D Resume3."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--mode", choices=("controller", "probe", "science"), default="controller"
    )
    ...
    from ccda_phase3.phase314b_r259_staged_resume3_variable_threshold_call_order_recovery import (
        BLOCKED_REPORT,
        PROBE_EVIDENCE,
        SUCCESS_REPORT,
        WORKER_EVIDENCE,
    ...
    if args.mode == "probe":
        ...
        try:
            ...
            print(json.dumps({"mode": "probe", "process_id": payload["process_id"]}))
            return 0
    ...
    write_ahead = (
        ...
        else root.parent / (root.name + ".phase314b_r259_staged_resume3_write_ahead")
    )
    try:
        ...
        print("BLOCKED: write-ahead must be outside Git worktree", file=sys.stderr)
        return 2
    ...
    if success.exists() or blocked.exists():
        print("BLOCKED: Resume3 terminal report already exists", file=sys.stderr)
        return 2
    ...
    try:
        ...
        if write_ahead.exists():
            raise RuntimeError("Resume3 write-ahead already exists")
        ...
        run_child(
            ...
            root=root,
            env=env,
            label="Stage-D Resume3 portable probe",
        )
        run_child(
            [
                args.python_bin,
            ...
            root=root,
            env=env,
            label="Stage-D Resume3 cold science worker",
        )

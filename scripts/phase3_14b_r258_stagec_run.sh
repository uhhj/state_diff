...
on_error() {
  ...
  cleanup_tmp
  "${PYTHON_BIN}" \
    "${ROOT}/scripts/phase3_14b_r258_stagec_blocked.py" \
    --root "${ROOT}" \
    --exit-code "${code}" \
...
print({
    ...
    "base_root":
        result["summary"]["root_cause"],
})
PY

PREFLIGHT_DIR="$(
  mktemp -d \
    /tmp/phase314b_r258_stagec_environment.XXXXXX
)"
PREFLIGHT="${PREFLIGHT_DIR}/environment.json"
if [[ -e "${PREFLIGHT}" ]]; then
  echo "Stage-C preflight output unexpectedly exists" >&2
  exit 1
fi
...
if [[ "${OBSERVED_STAGED}" != "${EXPECTED_STAGED}" ]]; then
  echo "Unexpected staged Stage-C implementation paths" >&2
  printf 'Expected:\n%s\nObserved:\n%s\n' \
    "${EXPECTED_STAGED}" \
    "${OBSERVED_STAGED}" >&2
  exit 1
...
UNEXPECTED="$(
  ...
  grep -v '^?? reports/phase3_14b_r258_stagec_test_gate_summary.json$' |
  grep -v '^A  ccda_phase3/phase314b_r258_stagec_balanced_geometry.py$' |
  grep -v '^A  scripts/phase3_14b_r258_stagec_worker.py$' |
  grep -v '^A  scripts/phase3_14b_r258_stagec_run_calibration.py$' |
  grep -v '^A  scripts/phase3_14b_r258_stagec_test_gate.py$' |
  grep -v '^A  scripts/phase3_14b_r258_stagec_blocked.py$' |
  grep -v '^A  scripts/phase3_14b_r258_stagec_run.sh$' |
  grep -v '^A  tests/test_phase3_14b_r258_stagec_balanced_geometry.py$' || true
)"

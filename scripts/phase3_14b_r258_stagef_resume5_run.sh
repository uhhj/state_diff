...
on_error() {
  ...
  if [[ "${PUSH_COMPLETED}" != "true" ]]; then
    "${PYTHON_BIN}" \
      "${ROOT}/scripts/phase3_14b_r258_stagef_resume5_blocked.py" \
      --root "${ROOT}" \
      --exit-code "${code}" \
...
[[ ! -e "${PREFLIGHT}" ]]
"${PYTHON_BIN}" scripts/phase3_14b_r258_stagef_resume5_worker.py \
...
"${PYTHON_BIN}" scripts/phase3_14b_r258_stagef_resume5_test_gate.py \
  --root "${ROOT}" --python-bin "${PYTHON_BIN}"
"${PYTHON_BIN}" scripts/phase3_14b_r258_stagef_resume5_run_calibration.py \
  --root "${ROOT}" --python-bin "${PYTHON_BIN}"

SUCCESS_PATHS=(
  reports/phase3_14b_r258_stagef_resume5_test_gate_summary.json
  reports/phase3_14b_r258_stagef_resume5_contract.json
  reports/phase3_14b_r258_stagef_resume5_worker_evidence.json
  reports/phase3_14b_r258_stagef_resume5_summary.json
  reports/phase3_14b_r258_stagef_resume5_report.md
)

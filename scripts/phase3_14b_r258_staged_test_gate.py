#!/usr/bin/env python3
"""Manifest-driven test gate for Phase3.14b-r2.5.8 Stage D."""
...
def main() -> None:
    ...
    if output.exists():
        raise FileExistsError(
            "refusing to overwrite Stage-D test gate: {}".format(output)
        )

    provenance = validate_base_evidence(
        root
    )
    ...
    if int(
        base.get("resume1_new_passed", -1)
    ) != 41:
        raise RuntimeError(
            "base Resume1 new-test count changed"
        )
    ...
    compile_targets = [
        ...
        / (
            "scripts/"
            "phase3_14b_r258_staged_test_gate.py"
        ),
        root
        / (
            "scripts/"
            "phase3_14b_r258_staged_blocked.py"
        ),
    ]
    subprocess.run(
        [
    ...
    temporary_root = Path(
        tempfile.mkdtemp(
            prefix="phase314b_r258_staged_gate_",
            dir="/tmp",
        )
    )
    try:
        frozen_tests = subprocess.run(
            [
                sys.executable,
                "-m",
                "pytest",
        ...
        if frozen_tests.returncode != 0:
            raise RuntimeError(
                "frozen 51-file test population failed"
            )
        base_passed = parse_pass_count(
            frozen_tests.stdout
        )
        if base_passed != 1064:
            raise RuntimeError(
                "frozen pass count changed: {}".format(base_passed)
            )
        ...
        if new_passed != 84:
            raise RuntimeError(
                "Stage-D new-test count changed: {}".format(new_passed)
            )
    finally:
        shutil.rmtree(
            temporary_root,
    ...
    total_passed = 1064 + new_passed
    if total_passed != 1148:
        raise RuntimeError(
            "Stage-D total pass count changed"
        )
    ...
    report: Dict[str, Any] = {
        ...
        "schema":
            "phase314b_r258_staged_test_gate_v1",
        "verdict": "PASS",
        "base_evidence_commit":
            BASE_EVIDENCE_COMMIT,
        "base_gate_path":
            BASE_TEST_GATE,
        "base_gate_report_bound_to_commit":
            True,
        "base_manifest_replayed_directly":
            True,
        "base_gate_script_rerun":
            False,
        "base_passed": 1064,
        "staged_new_passed":
            new_passed,

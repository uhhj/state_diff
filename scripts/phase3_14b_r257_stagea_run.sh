...
on_error() {
  local code="$?"
  "${PYTHON_BIN}" \
    "${ROOT}/scripts/phase3_14b_r257_stagea_blocked.py" \
    --root "${ROOT}" \
    --exit-code "${code}" \
...
if [[ "${OBSERVED_STAGED}" != "${EXPECTED_STAGED}" ]]; then
  echo "Unexpected staged r2.5.7 implementation paths" >&2
  printf 'Expected:\n%s\nObserved:\n%s\n' \
...
UNEXPECTED="$(
  ...
  grep -v '^?? reports/phase3_14b_r257_stagea_test_gate_summary.json$' |
  grep -v '^A  ccda_phase3/phase314b_r257_stagea_upper_objective.py$' |
  grep -v '^A  scripts/phase3_14b_r257_stagea_worker.py$' |
  grep -v '^A  scripts/phase3_14b_r257_stagea_run_calibration.py$' |
  grep -v '^A  scripts/phase3_14b_r257_stagea_test_gate.py$' |
  grep -v '^A  scripts/phase3_14b_r257_stagea_blocked.py$' |
  grep -v '^A  scripts/phase3_14b_r257_stagea_run.sh$' |
  grep -v '^A  tests/test_phase3_14b_r257_stagea_upper_objective.py$' || true
)"
if [[ -n "${UNEXPECTED}" ]]; then
  echo "Unexpected worktree changes before r2.5.7 implementation commit" >&2
  printf '%s\n' "${UNEXPECTED}" >&2
  exit 1
fi

git commit -m \
  "Add Phase3.14b-r2.5.7 upper-expansion objective calibration"

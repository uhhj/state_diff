#!/usr/bin/env python3
"""Run two isolated r2.5.7 Stage-D workers and seal evidence."""
...
TEST_GATE = "reports/phase3_14b_r257_staged_test_gate_summary.json"
DEFAULT_CONTRACT = "reports/phase3_14b_r257_staged_timestep_gate_contract.json"
DEFAULT_WORKER = "reports/phase3_14b_r257_staged_worker_evidence.json"
DEFAULT_SUMMARY = "reports/phase3_14b_r257_staged_summary.json"
DEFAULT_REPORT = "reports/phase3_14b_r257_staged_report.md"
...
def assert_repository(root: Path, test_gate: Path) -> Dict[str, Any]:
    if git_output(root, "branch", "--show-current") != "Experiment1":
        raise RuntimeError("r2.5.7 Stage D requires Experiment1")
    ...
    if unexpected:
        raise RuntimeError(
            "unexpected worktree paths before Stage-D run: " + repr(unexpected)
        )
    return {
        "branch": "Experiment1",
...
def render_report(summary: Dict[str, Any]) -> str:
    ...
    calibration = result["calibration_contract"]["calibration"]
    ...
    lines = [
        ...
        "",
        "## Immutable Stage-C binding",
        "",
        (
            "- Stage-C worker identity SHA256: `"
            + result["immutable_inputs"]["stagec_worker_identity_sha256"]
            + "`"
        ),
        (
            "- Stage-C contract SHA256: `"
            + result["immutable_inputs"]["stagec_contract_sha256"]
            + "`"
        ),
        (
            "- Stage-C contract file SHA256: `"
            + result["immutable_inputs"]["stagec_contract_file_sha256"]
            + "`"
        ),
        (
            "- Stage-C selection SHA256: `"
            + result["immutable_inputs"]["stagec_selection_sha256"]
            + "`"
        ),
        "",
        "## Timestep-gate contract",
        "",
        (
            "- Contract SHA256: `"
            + result["calibration_contract"]["contract_sha256"]
            + "`"
        ),
        (
        ...
        "- Objective: `K16 quadratic raw-log excess`",
        "- Gate: `sampled timestep <= cutoff`",
        "- Gate normalization: `conditional mean over active rows`",
        "- Cutoffs: `10, 25, 50`",
        "- Target gradient ratios: `0.5, 1.0`",
        ...
        "",
        (
            "| Candidate | Cutoff | Target ratio | Lambda | Active rate | "
            "t10/t25/t50 excess reduction | Train ratio | Binary t10 | Eligible |"
        ),
        "|---|---:|---:|---:|---:|---|---:|---:|---:|",
    ]
    for record in selection["candidate_records"]:
        candidate = record["candidate"]
        diagnostics = record["training_diagnostics"]
        response = record["continuous_response"]
        lines.append(
            "| "
            f"{record['candidate_id']} | "
            f"{candidate['timestep_cutoff']} | "
            f"{candidate['target_gradient_ratio']:.6g} | "
            f"{candidate['lambda_upper']:.12g} | "
            f"{diagnostics['observed_global_active_fraction']:.9g} | "
            f"{response['t10_mean_excess_reduction']:.4%} / "
            f"{response['t25_mean_excess_reduction']:.4%} / "
            f"{response['t50_mean_excess_reduction']:.4%} | "
    ...
    lines.extend(
        [
            ...
            (
                ...
                + classification["best_candidate_by_t10_continuous_excess"]
                + "`"
            ),
            (
            ...
            (
                "- Best fidelity candidate: `"
                + classification["best_fidelity_candidate"]
                + "`"
            ),
            (
                "- Best train-control NMSE ratio: `"
                f"{classification['best_train_control_nmse_ratio']:.9g}`"
            ),
            "",
            "## Classification",
            "",
            ...
            "## Boundary",
            "",
            "- Stage-C source, tests, contracts and evidence were immutable.",
            "- One exact diffusion-only control and six train-only timestep-gated K16 candidates were evaluated.",
            "- Candidate lambda values were calibrated on a deterministic batch containing t=0 through t=99 exactly once.",
...
def main() -> None:
    ...
    for path in (contract_path, worker_path, summary_path, report_path):
        if path.exists():
            raise FileExistsError(f"r2.5.7 Stage-D write-once output exists: {path}")

    repository = assert_repository(root, test_gate_path)
    test_gate = load_json(test_gate_path)
    if test_gate.get("verdict") != "PASS":
        raise RuntimeError("r2.5.7 Stage-D test gate is not PASS")

    temporary_root = Path(
        tempfile.mkdtemp(prefix="phase314b_r257_staged_", dir="/tmp")
    )
    worker_files = [temporary_root / "worker_1.json", temporary_root / "worker_2.json"]
    command = [
        str(args.python_bin),
        str(root / "scripts/phase3_14b_r257_staged_worker.py"),
        "--root",
        str(root),
    ]
    ...
    if not comparison["exact"]:
        raise RuntimeError("isolated r2.5.7 Stage-D workers differ")
    ...
    worker_evidence = {
        "phase": PHASE,
        "schema": "phase314b_r257_staged_worker_evidence_v1",
        "worker_count": 2,
        "workers_exact": True,
    ...
    summary = {
        "phase": PHASE,
        "schema": "phase314b_r257_staged_summary_v1",
        "verdict": "PASS",

...
def scan_scripts(root: Path) -> List[Dict[str, Any]]:
    ...
    targets = [
        ...
        root / "scripts/phase3_11_analyze_future_source_swap.py",
    ]
    ...
    for path in targets:
        ...
        for term in forbidden_input_terms:
            for match in re.finditer(term, txt):
                ...
                if any(k in ctx for k in ["EPISODE_FIELDS", "STEP_FIELDS", "_FIELDS =", "Do not concatenate"]):
                    continue
...
def main() -> None:
    ...
    parser.add_argument("--phase310b_summary", default="reports/phase3_10b_future_rollout_audit_summary.json")
    parser.add_argument("--best_ablation", default="xy_only_high_weight")
    parser.add_argument("--out_json", default="reports/phase3_11_future_source_swap_preflight_summary.json")
    parser.add_argument("--out_md", default="reports/phase3_11_future_source_swap_preflight_report.md")
    ...
    # Use the existing TensorFlow-free DeformableRavens runtime loader so
    # ravens.tasks / ravens.environment can be imported without executing
    # top-level ravens imports that pull TensorFlow-only agent/model modules.
    try:
    ...
    p310b = load_json(root / args.phase310b_summary)
    checks["phase39b_summary_exists"] = bool(p39b)
    checks["phase39b_best_matches"] = p39b.get("best_ablation") == args.best_ablation
    checks["phase310b_summary_exists"] = bool(p310b)
    checks["phase310b_future_branch_mismatch"] = p310b.get("root_cause") == "phase310b_predicted_future_branch_mismatch_supported"
    checks["phase310b_no_phase4_scope"] = (root / "reports/phase3_10b_no_phase4_confirmation.md").exists()

    for key in [
        "phase39b_summary_exists",
        "phase39b_best_matches",
        "phase310b_summary_exists",
        "phase310b_future_branch_mismatch",
        "phase310b_no_phase4_scope",
    ]:
        if not checks[key]:
            issue("FAIL", key, key)
    ...
    payload = {
        ...
        "phase310b_summary": {
            "verdict": p310b.get("verdict"),
            "root_cause": p310b.get("root_cause"),
            "diagnostic_flags": p310b.get("diagnostic_flags"),
        },
    }
    ...
    lines = [
        "# Phase3.11 Future-Source Swap Preflight",
        "",
        "## Verdict",
        "",
        f"- Verdict: `{verdict}`",
    ...
    lines += [
        ...
        "- Phase3.11 runs future-source swap audit only.",
        "- No model training.",
    ...
    if verdict == "FAIL":
        raise SystemExit("[Phase3.11][FAIL] preflight failed")

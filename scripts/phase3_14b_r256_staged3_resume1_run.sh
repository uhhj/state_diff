...
on_error() {
  local code="$?"
  "${PYTHON_BIN}" \
    "${ROOT}/scripts/phase3_14b_r256_staged3_resume1_blocked.py" \
    --root "${ROOT}" \
    --exit-code "${code}" \
...
IMPLEMENTATION_PATHS=(
  ...
  scripts/phase3_14b_r256_staged3_resume1_run.sh
...
UNEXPECTED="$(
  ...
  grep -v '^?? reports/phase3_14b_r256_staged3_resume1_test_gate_summary.json$' |
  grep -v '^A  ccda_phase3/phase314b_r256_staged3_resume1_upper_gate_freeze.py$' |
  grep -v '^A  scripts/phase3_14b_r256_staged3_resume1_worker.py$' |
  grep -v '^A  scripts/phase3_14b_r256_staged3_resume1_run_freeze.py$' |
  grep -v '^A  scripts/phase3_14b_r256_staged3_resume1_test_gate.py$' |
  grep -v '^A  scripts/phase3_14b_r256_staged3_resume1_blocked.py$' |
  grep -v '^A  scripts/phase3_14b_r256_staged3_resume1_run.sh$' |
  grep -v '^A  tests/test_phase3_14b_r256_staged3_resume1_source_predicate.py$' || true
)"
...
git commit -m "Fix Phase3.14b-r2.5.6 Stage D.3 source predicate"

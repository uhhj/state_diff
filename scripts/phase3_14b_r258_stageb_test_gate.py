#!/usr/bin/env python3
"""Manifest-driven test gate for Phase3.14b-r2.5.8 Stage B."""
...
BASE_GATE_SCRIPT = (
    "scripts/"
    "phase3_14b_r258_stagea_resume1_test_gate.py"
)
NEW_TEST = (
    "tests/"
    "test_phase3_14b_r258_stageb_direct_x0_reachability.py"
)
...
def main() -> None:
    ...
    if output.exists():
        raise FileExistsError(
            "refusing to overwrite Stage-B "
            "test gate: {}".format(output)
        )
    ...
    if int(
        ...
        raise RuntimeError(
            "base Resume1 test count changed"
        )
    ...
    compile_targets = [
        ...
        / (
            "ccda_phase3/"
            "phase314b_r258_stageb_direct_x0_reachability.py"
        ),
        root
        / (
            "scripts/"
        ...
        / (
            "scripts/"
            "phase3_14b_r258_stageb_run_audit.py"
        ),
        root
        / (
            "scripts/"
        ...
        / (
            "scripts/"
            "phase3_14b_r258_stageb_blocked.py"
        ),
    ]
    subprocess.run(
        [
    ...
    temporary_root = Path(
        tempfile.mkdtemp(
            prefix="phase314b_r258_stageb_gate_",
            dir="/tmp",
        )
    )
    base_replay_output = (
    ...
    try:
        ...
        if replay.returncode != 0:
            raise RuntimeError(
                "base Stage-A Resume1 "
                "test gate replay failed"
            )
    ...
    total_passed = 911 + new_passed
    if total_passed != 959:
        raise RuntimeError(
            "Stage-B total pass count changed"
        )
    ...
    report: Dict[str, Any] = {
        ...
        "schema":
            "phase314b_r258_stageb_test_gate_v1",
        "verdict": "PASS",
        "base_evidence_commit":
            BASE_EVIDENCE_COMMIT,
        "base_gate_path":

...
def build_feature_specs(
    *,
    ...
    for noise_std in (0.00025, 0.0005, 0.001):
        label = str(noise_std).replace("0.", "")
        specs.append(
            FeatureSpec(
                f"model_x_v2_s5_noise_{label}m",
                True,
                "noisy_visual_proxy",
                lambda tr, step, seed, track, noise_std=noise_std: build_model_x_v2(
                    add_deterministic_xy_noise(
                        tr["bead_xy"],
                        std_m=noise_std,
                        key=(
                            f"r23|seed={seed}|track={track}|std={noise_std}"
                        ),
                    ),
                    tr["robot_proxy"],
                    step=step,
...
def main() -> None:
    ...
    parser.add_argument(
        "--data",
        default="data/phase3_12d_r23_observation_contract/trajectory_observations.npz",
    )
    parser.add_argument(
        "--primary-steps",
        nargs="+",
    ...
    parser.add_argument(
        "--out-prefix",
        default="reports/phase3_12d_r23",
    )
    ...
    comparisons = [
        (f"primary_step_{step}", "free_a", "hidden_armed", step)
        for step in primary_steps
    ]
    comparisons += [
        ("control_free_replicate", "free_a", "free_b", decision_step),
        (
            "control_hidden_unarmed",
            "free_a",
            "hidden_unarmed",
            decision_step,
        ),
        ("control_horizon0", "free_a", "hidden_armed", 0),
    ]
    ...
    for comparison, left, right, step in comparisons:
        for spec_index, spec in enumerate(specs):
            ...
            print(
                f"[r2.3] {comparison} step={step} {spec.name} "
    ...
    privileged = [
        row
        for row in primary
        if row["feature_family"] == "privileged_velocity"
        and supported(row)
    ]
    robot_leak = [
        row
        for row in primary
        if row["feature_family"] == "robot_proprio"
        and supported(row)
    ]
    observable_motion = [
        row
        for row in primary
        if row["legal_observation"]
        and row["feature_family"]
        in {
            "observable_motion",
            "proposed_model_input",
            "noisy_visual_proxy",
    ...
    legal_rows = [row for row in primary if row["legal_observation"]]
    legal_below = bool(legal_rows) and all(
        row["roc_auc"] <= 0.60
    ...
    if control_failures:
        verdict = "FAIL"
        root_cause = "phase312d_r23_collection_or_statistics_invalid"
    elif robot_leak:
        verdict = "FAIL"
        root_cause = "phase312d_r23_robot_proprio_leakage_supported"
    elif observable_motion:
        verdict = "FAIL"
        root_cause = (
            "phase312d_r23_latent_condition_leaks_through_observable_motion"
        )
    elif privileged and legal_below:
        verdict = "WARN"
        root_cause = "phase312d_r23_privileged_velocity_is_primary_leakage"
    else:
        verdict = "WARN"
        root_cause = "phase312d_r23_observation_contract_inconclusive"
    ...
    payload = {
        ...
        "supported_privileged_features": [
            row["feature"] for row in privileged
        ],
        "supported_robot_features": [
            row["feature"] for row in robot_leak
        ],
        "supported_observable_motion_features": [
            row["feature"] for row in observable_motion
        ],
        "all_legal_features_below_identifiability": legal_below,
        "proposed_schema": {
        ...
        "scientific_boundary": (
            "This is an observation-contract diagnostic. It does not "
            "train StateDiff/IDM, execute the candidate matrix, prove "
            "CPS effectiveness, or authorize Phase4."
        ),
        "next_step": (
            "If only privileged velocity is supported, generate a "
            "versioned state-v2 dataset and retrain all baselines. "
            "If legal causal history is supported, design a versioned "
            "slack-breakaway hidden condition."
        ),
    }

    prefix = root / args.out_prefix
    strict_json_dump(
        Path(str(prefix) + "_observation_contract_summary.json"),
        payload,
    )
    write_csv(
        Path(str(prefix) + "_feature_summary.csv"),
        summary_rows,
        list(summary_rows[0]),
    )
    write_csv(
        Path(str(prefix) + "_fold_metrics.csv"),
        fold_rows_all,
        list(fold_rows_all[0]),
    )
    write_csv(
        Path(str(prefix) + "_predictions.csv"),
        prediction_rows,
    ...
    Path(str(prefix) + "_observation_contract_report.md").write_text(
        "\n".join(lines) + "\n"
    )

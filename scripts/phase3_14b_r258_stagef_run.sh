...
commit_with_outcome() {
  ...
  log_path="$(mktemp /tmp/phase314b_r258_stagef_commit.XXXXXX.log)"
...
push_with_outcome() {
  ...
  log_path="$(mktemp /tmp/phase314b_r258_stagef_push.XXXXXX.log)"
...
bash -n scripts/phase3_14b_r258_stagef_run.sh

PREFLIGHT_DIR="$(mktemp -d /tmp/phase314b_r258_stagef_environment.XXXXXX)"
PREFLIGHT="${PREFLIGHT_DIR}/environment.json"
if [[ -e "${PREFLIGHT}" ]]; then
  printf 'Stage-F preflight output unexpectedly exists\n' >&2
  exit 1
fi
"${PYTHON_BIN}" \
  scripts/phase3_14b_r258_stagef_worker.py \
...
SUCCESS_PATHS=(
  ...
  reports/phase3_14b_r258_stagef_contract.json
  reports/phase3_14b_r258_stagef_worker_evidence.json
  reports/phase3_14b_r258_stagef_summary.json
  reports/phase3_14b_r258_stagef_report.md
)
...
trap - ERR
printf 'Stage F complete and pushed: %s\n' "${LOCAL_HEAD}"

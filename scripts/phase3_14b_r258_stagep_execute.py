#!/usr/bin/env python3
"""Execute Stage-P oracle upper-segment gate provenance exactly once."""
...
from ccda_phase3.phase314b_r258_stagep_oracle_upper_segment_gate_provenance import (
    BLOCKED_REPORT,
    SUCCESS_REPORT,
    StagePError,
    blocked_report,
    run_upper_gate_provenance,
    stable_json_bytes,
...
def load_json(path: Path) -> Mapping[str, Any]:
    ...
    if not isinstance(value, dict):
        raise StagePError("environment payload is not a mapping")
    return value
...
def write_once(path: Path, payload: bytes) -> None:
    target = Path(path)
    if target.exists():
        raise StagePError(f"write-once output already exists: {target}")
...
def main() -> int:
    ...
    if success.exists() or blocked.exists():
        print("BLOCKED: Stage-P output already exists", file=sys.stderr)
        return 2
    ...
    try:
        ...
        result = run_upper_gate_provenance(
            root=root,

"""Run the R2 gravity-and-floor table settle audit."""
...
from state_diff.env.block_pushing.table_settle_coupon import (  # noqa: E402
    TableSettleCoupon)


def run_settle(config_path: str, profile_path: str, selection_path: str) -> Path:
    """Run and save one table-settle trajectory at outer-step rate."""
    ...
    if not video.isOpened():
        coupon.close()
        raise RuntimeError("unable to write table-settle video")
    ...
    try:
        ...
        write_json(output / "metadata.json", {
            ...
            "node_count": len(block.body_ids), "all_nodes_dynamic": all(
                coupon.client.getDynamicsInfo(body, -1)[0] > 0 for body in block.body_ids),

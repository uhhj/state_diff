"""Run one R3 edge-angle material coupon."""
import argparse
...
from state_diff.env.block_pushing.angle_elastic_coupon import (  # noqa: E402
    AngleElasticMaterialCoupon)
from state_diff.env.block_pushing.material_coupon_r2 import COUPON_MODES  # noqa: E402


def run_coupon(config_path: str, profile_path: str, selection_path: str,
               mode: str) -> Path:
    config = load_config(config_path)
    _, _, profile = load_profile(profile_path)
    selection = load_selected_microsteps(selection_path)
    output = Path(config["output_root"]) / "coupon" / profile["profile_name"] / mode
    output.mkdir(parents=True, exist_ok=True)
    coupon = AngleElasticMaterialCoupon(
    ...
    try:
        ...
        write_json(output / "metadata.json", {
            ...
            "microstep_selection": selection, "anchor_mode": "static_nodes",
            "fixture_constraint_count": 0, "gravity": [0, 0, 0],
            "floor_loaded": False, "node_count": len(block.body_ids),
            ...
            "static_anchor_masses": [coupon.client.getDynamicsInfo(
                block.body_ids[int(i)], -1)[0] for i in coupon.anchor_indices],
            "dynamic_node_mass": coupon.client.getDynamicsInfo(
                block.body_ids[int(coupon.load_indices[0])], -1)[0],

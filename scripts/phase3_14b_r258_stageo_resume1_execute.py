#!/usr/bin/env python3
"""Execute Stage-O Resume1 descending internal-scale recovery exactly once."""
...
from ccda_phase3.phase314b_r258_stageo_resume1_descending_attempt_order_recovery import (
    BLOCKED_REPORT,
    SUCCESS_REPORT,
    StageOResume1Error,
    blocked_report,
...
def load_json(path: Path) -> Mapping[str, Any]:
    ...
    if not isinstance(value, dict):
        raise StageOResume1Error("environment payload is not a mapping")
    return value

def write_once(path: Path, payload: bytes) -> None:
    target = Path(path)
    if target.exists():
        raise StageOResume1Error(f"write-once output already exists: {target}")
...
def main() -> int:
    ...
    if success.exists() or blocked.exists():
        print("BLOCKED: Stage-O Resume1 output already exists", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(json.dumps({
            ...
            "stage_o_result_sha256": result["stage_o_result_sha256"],
            "output": str(success),
        }, sort_keys=True))
        return 0

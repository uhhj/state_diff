#!/usr/bin/env python3
"""Manifest-driven static test gate for Phase3.14b-r2.5.5 Stage C."""
...
STAGE_B_GATE = "reports/phase3_14b_r255_stageb_test_gate_summary.json"
STAGE_A_TEST = "tests/test_phase3_14b_r255_robot_proxy_provenance.py"
STAGE_B_TEST = "tests/test_phase3_14b_r255_stageb_dataset.py"
STAGE_C_TEST = "tests/test_phase3_14b_r255_stagec_cache_attribution.py"
EXPECTED_HISTORICAL_PASSED = 425
EXPECTED_STAGE_A_PASSED = 6
EXPECTED_STAGE_B_PASSED = 14
EXPECTED_STAGE_C_PASSED = 18
EXPECTED_TOTAL_PASSED = 463
...
def run_pytest(
    *,
    ...
    command = [python_bin, "-m", "pytest", "-q"] + list(files)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        default="reports/phase3_14b_r255_stagec_test_gate_summary.json",
    )
    parser.add_argument(
        "--python-bin",
    ...
    stage_b_gate_path = root / STAGE_B_GATE
    if sha256_file(stage_b_gate_path) != EXPECTED_STAGE_B_TEST_GATE_SHA256:
        raise RuntimeError("Stage B test gate SHA changed")
    stage_b = load_json(stage_b_gate_path)
    if stage_b.get("verdict") != "PASS":
        raise RuntimeError("Stage B test gate is not PASS")
    if int(stage_b.get("passed_test_count", -1)) != 445:
        raise RuntimeError("Stage B pass count changed")
    frozen_manifest = {
        str(key): str(value)
        for key, value in stage_b["test_manifest_sha256"].items()
    }
    if len(frozen_manifest) != 29:
        raise RuntimeError("Stage B frozen test file count changed")
    verify_manifest(root, frozen_manifest)
    if STAGE_A_TEST not in frozen_manifest or STAGE_B_TEST not in frozen_manifest:
        raise RuntimeError("Stage A/B tests missing from frozen manifest")
    if not (root / STAGE_C_TEST).is_file():
        raise FileNotFoundError(root / STAGE_C_TEST)
    ...
    subprocess.run(
        ["bash", "-n", str(root / "scripts/phase3_14b_r255_stagec_run.sh")],
        cwd=str(root),
        check=True,
    )
    ...
    historical = sorted(
        ...
        if relative not in {STAGE_A_TEST, STAGE_B_TEST}
    )
    worktree = root.parent / f".phase314b_r255_stagec_baseline_{os.getpid()}"
    if worktree.exists():
    ...
    stage_a_passed, _ = run_pytest(
        python_bin=args.python_bin,
        cwd=root,
        files=[STAGE_A_TEST],
    )
    stage_b_passed, _ = run_pytest(
        python_bin=args.python_bin,
        cwd=root,
        files=[STAGE_B_TEST],
    )
    stage_c_passed, _ = run_pytest(
        python_bin=args.python_bin,
        cwd=root,
        files=[STAGE_C_TEST],
    )
    total = historical_passed + stage_a_passed + stage_b_passed + stage_c_passed
    observed = (
        historical_passed,
        stage_a_passed,
        stage_b_passed,
    ...
    expected = (
        ...
        EXPECTED_STAGE_A_PASSED,
        EXPECTED_STAGE_B_PASSED,
        EXPECTED_STAGE_C_PASSED,
        EXPECTED_TOTAL_PASSED,
    )
    if observed != expected:
        raise RuntimeError(f"test pass-count contract changed: {observed} != {expected}")

    current_manifest = dict(frozen_manifest)
    current_manifest[STAGE_C_TEST] = sha256_file(root / STAGE_C_TEST)
    if len(current_manifest) != EXPECTED_TEST_FILES:
        raise RuntimeError("Stage C test file count changed")
    payload: Dict[str, Any] = {
        "phase": "Phase3.14b-r2.5.5 Stage C",
        "schema": "phase314b_r255_stagec_manifest_driven_test_gate_v1",
        "verdict": "PASS",
        "stage_b_test_gate_sha256": EXPECTED_STAGE_B_TEST_GATE_SHA256,

...
def load_codec(root: Path, data: Any, fallback: str) -> tuple[Any, str, Dict[str, Any]]:
    ...
    for cand in candidates:
        ...
        try:
            ...
            with path.open("rb") as f:
                codec = pickle.load(f)
        return codec, str(path), codec_summary(codec)
...
def checkpoint_summary(root: Path, checkpoint_root: str) -> Dict[str, Any]:
    ...
    for baseline in ["paper_state", "state_action"]:
        bdir = ckpt_root / baseline
        dirs = sorted([p for p in bdir.glob("fold_*_seed_*") if p.is_dir()]) if bdir.exists() else []
        complete = []
        for d in dirs:
            if (d / "state_model.pt").exists() and (d / "inverse_dynamics.pt").exists():
...
def main() -> None:
    ...
    parser.add_argument("--out_json", default="reports/phase3_8_idm_geometry_preflight_summary.json")
    parser.add_argument("--out_md", default="reports/phase3_8_idm_geometry_preflight_report.md")
    ...
    codec, codec_path, codec_sum = load_codec(root, data, args.action_template)
    checks["action_template_exists"] = True
    checks["action_codec_dim_14"] = int(codec_dim(codec)) == 14
    checks["action_codec_no_camera_config"] = int(codec_sum.get("num_camera_config_paths", 0) or 0) == 0
    if not checks["action_codec_dim_14"]:
        issue("FAIL", "action_codec_dim_not_14", codec_sum)
    if not checks["action_codec_no_camera_config"]:
        issue("FAIL", "action_codec_has_camera_config", codec_sum)
    ...
    if p37_path.exists():
        p37 = json.loads(p37_path.read_text())
        checks["phase37_summary_exists"] = True
        checks["phase37_expected_root_cause"] = p37.get("root_cause") == "idm_action_execution_geometry_blocker"
        checks["phase37_verdict_fail_expected"] = p37.get("verdict") == "FAIL"
        checks["phase37_gt_progress_positive"] = float(p37.get("gt_exec_mean_delta_final_fraction", 0.0) or 0.0) > 0.0
        checks["phase37_idm_gt_no_progress"] = abs(float(p37.get("idm_gt_exec_mean_delta_final_fraction", 999.0) or 999.0)) < 1e-6
    else:
        p37 = {}
        checks["phase37_summary_exists"] = False
        checks["phase37_expected_root_cause"] = False
        checks["phase37_verdict_fail_expected"] = False
        checks["phase37_gt_progress_positive"] = False
        checks["phase37_idm_gt_no_progress"] = False
        issue("FAIL", "phase37_summary_missing", str(p37_path))
    ...
    if p36_path.exists():
        ...
        checks["phase36_state_match"] = False
    ...
    payload = {
        ...
        "phase37_summary": {
            ...
            "idm_pred_exec_mean_delta_final_fraction": p37.get("idm_pred_exec_mean_delta_final_fraction"),
        },
    }
    ...
    if verdict == "FAIL":
        raise SystemExit("[Phase3.8][FAIL] preflight failed")

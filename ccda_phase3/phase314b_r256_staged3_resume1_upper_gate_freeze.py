"""Phase3.14b-r2.5.6 Stage D.3 Resume1 upper-gate replay.
...
PHASE_ID = "phase314b_r256_staged3_resume1"
BASE_EVIDENCE_COMMIT = "306575ee46edd9d163550ad310544f10ed1258fe"
BLOCKED_IMPLEMENTATION_COMMIT = "7713f9feb07d2053744dc67819cd59682c03370a"
EXPECTED_SUBMODULE_COMMIT = "633a88752445cf5d6776ed374fdbbdb35f93050c"

BLOCKED_TEST_GATE = "reports/phase3_14b_r256_staged3_test_gate_summary.json"
BLOCKED_SUMMARY = "reports/phase3_14b_r256_staged3_blocked_summary.json"
EXPECTED_BLOCKED_TEST_GATE_SHA256 = (
    "210bd16acc348bfa1d90df016083be490fe9f28b588e5ab9cd80b42c956a9c9b"
)
EXPECTED_BLOCKED_SUMMARY_SHA256 = (
    "dd53e4170ed8d33f29fcbb3e425c41c3e21ee57cc138ffabb469a029c90066cd"
)
BLOCKED_IMPLEMENTATION_FILES = (
    "ccda_phase3/phase314b_r256_staged3_upper_gate_freeze.py",
    "scripts/phase3_14b_r256_staged3_worker.py",
    "scripts/phase3_14b_r256_staged3_run_freeze.py",
    "scripts/phase3_14b_r256_staged3_test_gate.py",
    "scripts/phase3_14b_r256_staged3_blocked.py",
    "scripts/phase3_14b_r256_staged3_run.sh",
...
class OneSidedUpperContract:
    ...
    def to_dict(self) -> Dict[str, Any]:
        self.validate()
        return {
            "schema": "phase314b_r256_staged3_resume1_one_sided_upper_contract_v1",
...
def validate_immutable_inputs(root: Path) -> Dict[str, Any]:
    ...
    blocked_attempt = validate_blocked_attempt(repository_root)
    return {
...
def assert_file_at_commit(
    ...
    if committed != observed:
        raise UpperGateFreezeError(
            f"file differs from committed blocked attempt: {relative}"
        )
    return sha256_bytes(observed)


def validate_blocked_test_gate_payload(
    payload: Mapping[str, Any],
) -> None:
    if payload.get("verdict") != "PASS":
        raise UpperGateFreezeError(
            "blocked D.3 test gate verdict changed"
        )
    if int(payload.get("passed_test_count", -1)) != 597:
        raise UpperGateFreezeError(
            "blocked D.3 pass count changed"
        )
    if int(payload.get("test_file_count", -1)) != 37:
        raise UpperGateFreezeError(
            "blocked D.3 test-file count changed"
        )
    if int(payload.get("staged3_new_passed", -1)) != 24:
        raise UpperGateFreezeError(
            "blocked D.3 new-test count changed"
        )


def validate_blocked_summary_payload(
    payload: Mapping[str, Any],
) -> None:
    if payload.get("verdict") != "BLOCKED":
        raise UpperGateFreezeError(
            "blocked D.3 verdict changed"
        )
    if payload.get("scientific_status") != "BLOCKED":
        raise UpperGateFreezeError(
            "blocked D.3 scientific status changed"
        )
    if payload.get("root_cause") != (
        "phase314b_r256_staged3_execution_failed_before_completion"
    ):
        raise UpperGateFreezeError(
            "blocked D.3 root cause changed"
        )
    if payload.get("gate_selected") is not False:
        raise UpperGateFreezeError(
            "blocked D.3 unexpectedly selected a gate"
        )
    if payload.get("threshold_recomputed") is not False:
        raise UpperGateFreezeError(
            "blocked D.3 unexpectedly recomputed a threshold"
        )
    if payload.get("train_only_recommendation") is not None:
        raise UpperGateFreezeError(
            "blocked D.3 selected a train-only recommendation"
        )
    if payload.get("selected_configuration") is not None:
        raise UpperGateFreezeError(
            "blocked D.3 selected a configuration"
        )


def validate_blocked_attempt(root: Path) -> Dict[str, Any]:
    repository_root = Path(root).resolve()
    ...
    implementation_sha = {
        relative: assert_file_at_commit(
            repository_root,
            BLOCKED_IMPLEMENTATION_COMMIT,
            relative,
        )
        for relative in BLOCKED_IMPLEMENTATION_FILES
    }

    test_gate_path = repository_root / BLOCKED_TEST_GATE
    blocked_path = repository_root / BLOCKED_SUMMARY
    for path in (test_gate_path, blocked_path):
        if not path.is_file():
            raise FileNotFoundError(path)
        subprocess.run(
            ["git", "ls-files", "--error-unmatch", str(path.relative_to(repository_root))],
            cwd=str(repository_root),
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
            check=True,
    ...
    if sha256_file(test_gate_path) != EXPECTED_BLOCKED_TEST_GATE_SHA256:
        raise UpperGateFreezeError(
            "blocked D.3 test-gate SHA changed"
        )
    if sha256_file(blocked_path) != EXPECTED_BLOCKED_SUMMARY_SHA256:
        raise UpperGateFreezeError(
            "blocked D.3 summary SHA changed"
        )

    test_gate = load_json(test_gate_path)
    blocked = load_json(blocked_path)
    validate_blocked_test_gate_payload(test_gate)
    validate_blocked_summary_payload(blocked)

    test_commit = git_output(
        repository_root,
        "log",
        "-1",
        "--format=%H",
    ...
    if not test_commit or test_commit != blocked_commit:
        raise UpperGateFreezeError(
            "blocked D.3 artifacts were not committed together"
        )
    for relative in (BLOCKED_TEST_GATE, BLOCKED_SUMMARY):
        committed = subprocess.check_output(
            ["git", "show", f"{blocked_commit}:{relative}"],
            cwd=str(repository_root),
        )
        if committed != (repository_root / relative).read_bytes():
            raise UpperGateFreezeError(
                f"blocked evidence differs from commit: {relative}"
            )
    ...
    if "raw_xy_float32_exact_required" not in blocked_source:
        raise UpperGateFreezeError(
            "blocked D.3 predicate typo is not reproduced"
        )

    return {
        "implementation_commit": BLOCKED_IMPLEMENTATION_COMMIT,
        "implementation_file_sha256": implementation_sha,
        "blocked_evidence_commit": blocked_commit,
        "test_gate_path": BLOCKED_TEST_GATE,
        "test_gate_sha256": EXPECTED_BLOCKED_TEST_GATE_SHA256,
        "blocked_summary_path": BLOCKED_SUMMARY,
        "blocked_summary_sha256": EXPECTED_BLOCKED_SUMMARY_SHA256,
        "test_gate": test_gate,
        "blocked_summary": blocked,
        "failure_stage": "source_logic_audit_before_threshold_or_model_replay",
        "predicate_typo": (
            "raw_xy_float32_exact_required -> "
            "cache_float32_exact_required"
        ),
    }


def d2_exactness_signature(stage_d2_text: str) -> bool:
    return bool(
        "cache_float32_exact_required: bool = True"
        in stage_d2_text
        and (
            "if spec.cache_float32_exact_required "
            "and not np.all(xy_exact):"
        )
        in stage_d2_text
...
def source_logic_audit(root: Path) -> Dict[str, Any]:
    ...
    checks = {
        ...
        "stage_d2_requires_raw_xy_cache_exact": (
            d2_exactness_signature(stage_d2_text)
        ),
    }
    return {
        "checks": checks,
...
def run_freeze_and_reaudit(
    *,
    ...
    return {
        "phase": PHASE,
        "phase_id": PHASE_ID,
        "schema": "phase314b_r256_staged3_resume1_worker_result_v1",
        "verdict": "PASS",
        ...
        "immutable_inputs": {
            ...
            "blocked_attempt": immutable["blocked_attempt"],
            "stage_d2_raw_xyz_sha256":
                immutable["stage_d2_raw_xyz_sha256"],
            "stage_d2_projection_artifact_fraction":
                immutable[
...
def identity_projection(
    ...
    return {
        ...
        "immutable_inputs": result["immutable_inputs"],

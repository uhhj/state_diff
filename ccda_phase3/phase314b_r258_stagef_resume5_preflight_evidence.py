"""Write-once preflight failure evidence for Stage F Resume5."""
...
BLOCKED_REPORT = REPO_ROOT / "reports/phase3_14b_r258_stagef_resume5_blocked_summary.json"


class Resume5BlockedReportError(RuntimeError):
    pass
...
def _safe_repo_state(repo: Path) -> Mapping[str, Any]:
    ...
    return {
        ...
        "origin_experiment1": _git(
            repo, "rev-parse", "refs/remotes/origin/Experiment1"
        ),
        ...
        "submodule_head": (
            _git(submodule, "rev-parse", "HEAD") if submodule.is_dir() else None
        ),
...
def write_resume5_preflight_blocked_report(
    *,
    failure_stage: str,
    root_cause: str,
    detail: str,
    ...
    """Create the Resume5 blocked report exactly once using O_EXCL."""
    ...
    try:
        ...
        raise Resume5BlockedReportError(
            f"refusing to overwrite existing Resume5 blocked report: {report_path}"
        ) from exc

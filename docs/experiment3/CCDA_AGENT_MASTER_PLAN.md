...
> **PB4-C0 current status (2026-08-14)**: Formal PB4 remains frozen as `PB4_CONTROL_RELEVANCE_NOT_CONFIRMED`. The unit-amplitude signed qpos-target grid was hard-limit infeasible in CPU preflight and was never promoted to protocol/GPU. PB4-C0 then preregistered and derived a CPU-only global feasibility-normalized family from frozen `best_qpos` and the pinned Panda URDF hard limits only: `alpha_max_hard_limits=0.26808133907545595`, `alpha_formal=0.25467727212168312`; the unit grid had 98 out-of-limit commands (114 joint-limit events), while the formal nine-action grid had zero hard-limit violations. This is not a Condition-5 test or confirmation; GPU was not executed. The next allowed work is a separately preregistered exploratory PB4-C1 GPU audit using this exact frozen alpha; it has not started. Deployable sensing and all model training remain unstarted.
...
> **当前科学状态**：Wiring-post 主路线已停止。DLO-Lab Wrapping 的 PB2-A/B、PB2-C、replay-floor/alignment 预注册以及 PB3 future bifurcation 均已完成。正式 PB4 仍冻结为 `PB4_CONTROL_RELEVANCE_NOT_CONFIRMED`。单位幅度 signed qpos-target 网格在 CPU 硬限位预检中不可行，未生成协议、未使用 GPU。PB4-C0 随后仅使用冻结的 `best_qpos` 与 pinned Panda URDF 硬限位，预注册并导出了全局可行性归一化族：`alpha_max_hard_limits=0.26808133907545595`、`alpha_formal=0.25467727212168312`；单位网格有 98 条越界命令（114 个关节限位事件），formal 九动作网格零越界。该结论是 CPU-only action-family derivation，不是 Condition 5 测试或确认，GPU 未执行。下一允许步骤是单独预注册、使用该精确冻结 α 的探索性 PB4-C1 GPU audit；本轮未启动它。deployable sensing、StateDiff/CFPM training 仍未启动。

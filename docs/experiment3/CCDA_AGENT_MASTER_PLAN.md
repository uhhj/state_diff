...
> **PB4-C1 current status (2026-08-14)**: The preregistered exploratory audit used the PB4-C0 family verbatim (`alpha_formal=0.25467727212168312`, nine actions, H=20) and completed its 20 branches × 9 actions × 3 repeats = 540-record matrix. All 10/10 fresh live pairs and all 60/60 snapshot restores passed first. The result is `PB4C1_NORMALIZED_QPOS_SIGNAL_NOT_FOUND`: no pair met the unchanged 0.05 unique-best and robust cross-regret criterion. This is an exploratory negative, not a Condition-5 confirmation. Stop the Wrapping control-relevance route: do not introduce another alpha, horizon, adaptive scale, clipping, or hand-designed rescue action. Deployable sensing and all model training remain unstarted.
...
> **当前科学状态**：Wiring-post 主路线已停止。DLO-Lab Wrapping 的 PB2-A/B、PB2-C、replay-floor/alignment 预注册以及 PB3 future bifurcation 均已完成。PB4-C1 探索性审计已逐字复用 PB4-C0 family（`alpha_formal=0.25467727212168312`、九动作、H=20），完成 20 branches × 9 actions × 3 repeats = 540 条记录；10/10 fresh live pair 与 20 个 native snapshot 的 60/60 restore 均在动作评价前通过。结论为 `PB4C1_NORMALIZED_QPOS_SIGNAL_NOT_FOUND`：无 pair 达到未改变的 0.05 unique-best 与 robust cross-regret criterion。这是探索性 negative，不是 Condition 5 确认。停止 Wrapping control-relevance route：不得再增加 alpha、horizon、adaptive scale、clipping 或手工 rescue action。deployable sensing、StateDiff/CFPM training 仍未启动。
